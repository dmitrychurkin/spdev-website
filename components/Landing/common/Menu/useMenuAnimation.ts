...
export default function useMenuAnimation(
  ...
  }: IUseMenuAnimationArgs = {}
) {
  const [fn, setFn] = useState<((args: IAnimationHandlerArgs) => void) | undefined>();
  useEffect(() => {
    ...
    if (nav instanceof HTMLElement && it.length > 0 && !fn) {
      ...
      setFn(() => ({ state, onAnimationEnd, options = animOptions }: IAnimationHandlerArgs) => {
        const nOptions = navbar.options ?? options;
        const mOptions = menuItem.options ?? options
        switch (state) {
          case MenuState.OPEN: {
            nav
              .animate(nKeyframes[state], nOptions)
              .onfinish = () => {
                let itemElAnim;
...
interface IUseMenuAnimationArgs {
  readonly navbar?: {
...
interface IAnimationHandlerArgs {
